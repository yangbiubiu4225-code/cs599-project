...
flowchart TD
    ...
    AG --> R
    R --> RAG["RAG Retriever Tool"]
    RAG --> KB["src/knowledge 音乐知识库"]
    RAG --> S["Spec Agent"]
    S --> P["Prompt Agent"]
...
sequenceDiagram
    ...
    RAG-->>Graph: retrieved_context / rag_sources
...
```mermaid
flowchart TB
    A["user_requirement / latest_user_message"] --> B["retrieved_context / rag_sources"]
    B --> C["music_spec"]
    C --> D["generation_prompt"]
    D --> E["audio_path / generation_result"]
    E --> F["audio_features"]
    F --> G["appreciation_result"]
    G --> H["revision_suggestion / optimized_prompt"]
    H --> I["history_id"]
```

图 3-4 MusicAgentState 数据流设计

核心状态字段：

表 3-1 MusicAgentState 核心字段

| 字段 | 用途 |
| --- | --- |
| `user_requirement` | 当前完整需求上下文 |
| `latest_user_message` | 本轮最新输入，优先级高于旧上下文 |
| `retrieved_context` | RAG 检索得到的音乐风格、鉴赏标准和 Prompt 示例上下文 |
| `rag_sources` | RAG 命中的知识库来源 |
| `rag_matches` | RAG 命中的片段、分数和文本 |
| `music_spec` | 结构化音乐规格 |
...
| RAG Retriever Tool | 检索风格与 Prompt 知识 | 检索风格与鉴赏知识 |
| Spec Agent | 使用 | 使用 |
...
图 6-1 LLM 增强后的 Agent 工作流

## 6.2 MusicGen 后端增强

当前 MusicGen 是可选本地后端，适合在具备模型环境时运行。未来可以增加：

表 6-1 MusicGen 后端增强方向
